...
Fix by dust:
1. Use 'fd' to find possilbe icons
2. Hand long text
"""
...
class Notifier(configurable.Configurable):
    ...
    defaults = [
        ...
        ('max_chars', 35, "Maximum chars in a single line."),
        ('max_lines', 3, "Maxinum number of lines in a single window."),
    ...
    def _send(
        ...
        tmps = text.splitlines()
        title = "<span font-weight=\"bold\">{}</span>".format(tmps[0])
        lines = []
        for each in tmps[1:]:
            if len(lines) >= self.max_lines:
                break
            while len(each) > self.max_chars and\
                  len(lines) < self.max_lines:
                lines.append(each[:self.max_chars])
                each = each[self.max_chars:]
            lines.append(each)
        if len(lines) >= self.max_lines:
            lines[self.max_lines-1] += "..."
        lines = ["<span foreground=\"{}\">{}</span>".format(self.foreground[0], each)\
                 for each in lines[:self.max_lines]]
    ...
    def _load_icon(self, notif: Notification) -> Optional[Tuple[ImageSurface, int]]:
        ...
        try:
            maybe_icons = subprocess.check_output(["fd", notif.app_icon, "/usr/share/icons/"]).decode()
            if not maybe_icons:
                return None
            img = images.Img.from_path(maybe_icons.splitlines()[0])
